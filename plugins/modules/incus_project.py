...
class IncusProject(object):
    ...
    def run_incus(self, args, check_rc=True):
        cmd = ['incus']
        # Project commands themselves don't usually take --project (they MANAGE projects).
        # Exception: incus list --project.
        # But 'incus project create' creates a project.
        # 'incus project edit <name>' edits it.
        # We don't need --project context usually when managing projects.
        cmd.extend(args)
    ...
    def run_incus_input(self, args, input_data):
        cmd = ['incus']
        # No --project needed for project management commands usually
        cmd.extend(args)
    ...
    def update(self, new_create=False):
        ...
        # Projects only have config and description (and name/used_by)
    ...
    def rename(self):
         ...
         # After rename, validation of config might be needed? 
         # Assuming rename implicitly keeps config.
        ...
        if self.state == 'present':
            if self.rename_from:
                ...
                if old_info and not current:
                    self.rename()
                    return # Exit after rename (could verify config too, but usually simple rename is enough for now)
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            # Projects usually don't have 'devices' at project level, config handles limits/features.
